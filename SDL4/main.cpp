...
#define WEIGHT	   240			     //chiều cao cửa sổ
...
int y = WEIGHT / 2, vy = 2;    //ban đầu của hình vuông 

void block(int x, int y)      // vẽ hình vuông 16x16 tại tạo độ (x,y)
{							 // sử dụng màu trắng
...
void init(int vsync)
{
	...
	wnd = SDL_CreateWindow("FPS", 100, 100, WWIDTH, WEIGHT, SDL_WINDOW_SHOWN);
...
int event()							// xử lý sự kiện
{
	...
	while (SDL_PollEvent(&e) != 0)  // lấy 1 sự kiện hỏi hàng đợi, nếu có
	{
		// nếu là phím nhấn, kiểm tra có phải ESC được nhấn hay không
		if (e.type == SDL_KEYDOWN)
			if (e.key.keysym.sym == SDLK_ESCAPE)
				running = 0;      // nếu đúng, ta yêu cầu đóng chương trình
		//nếu là sự kiện đóng cửa số (Nhấn vào nút [X])

		if (e.type == SDL_QUIT)
			running = 0;   // ta yêu cầu đóng chương trình
	}
	return running;
}

void update() // cập nhật trạng thái mới cho chương trình
{
	x += vx, y += vy;     // di chuyển hình vuông

	// nảy ngược lại nếu chạm các cạnh cửa sổ 
	if (x < 0 || x > WWIDTH - BLK_SIZE) vx = -vx;
	if (y < 0 || y > WEIGHT - BLK_SIZE)   vy = -vy;
}

void draw()			// xử lý thao tác dụng hình
{
	//xóa toàn bộ framebuffer về màu đen
	SDL_SetRenderDrawColor(rend, 0, 0, 0, 0);
	SDL_RenderClear(rend);
	block(x, y);				// vẽ hình vuông tạo vị trí hiện hành
...
int main(int argc, char** argv)
{
	...
	timerBegin = SDL_GetTicks();// lấy thời gian vòng lặp
	while (running)
	{
