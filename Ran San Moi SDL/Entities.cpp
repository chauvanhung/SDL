...
void snakeCell(COORD c, SDL_bool head)
{
	...
	rectDraw(c.x * BOARD_CELL, c.y * BOARD_CELL, BOARD_CELL, color);
}
...
void fruitGent(FRUIT* f, const SNAKE* s)
{
	...
	while (!ok)
	{
		...
		{
			if (f->loc.x != s->coords[i].x && f->loc.y != s->coords[i].y)
				continue;
		}
		if (i == s->len)
			ok = SDL_TRUE;
...
{
	rectDraw(f->loc.x * BOARD_CELL, f->loc.y * BOARD_CELL, BOARD_CELL, f->color);
